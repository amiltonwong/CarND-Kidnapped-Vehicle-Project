...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
  ...
  // define sensor noise's normal distributions
  ...
  for (int i = 0; i < num_particles; i++) {
    ...
    // add the noise
    p.x += N_x_init(rand_gen);
    p.y += N_y_init(rand_gen);
    p.theta += N_theta_init(rand_gen);
...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate) {
  ...
  for (int i = 0; i < num_particles; i++) {
    ...
    // add the noise
    particles[i].x += N_x(rand_gen);
    particles[i].y += N_y(rand_gen);
    particles[i].theta += N_theta(rand_gen);
  }
}
...
void ParticleFilter::dataAssociation(std::vector<LandmarkObs> predicted, std::vector<LandmarkObs>& observations) {
  ...
  for (unsigned int i = 0; i < observations.size(); i++) {
    ...
    // initialize the minimum distance to possible maximum
    double min_dist = numeric_limits<double>::max();

    // initialize the id of landmark from map placeholder to be associated with the observation
    int map_id = -1;
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
	...
	//   https://www.willamette.edu/~gorr/classes/rand_generalGraphics/Transforms/transforms2d.htm
...
void ParticleFilter::resample() {
  ...
  // rand_generate random starting index for resampling wheel
  uniform_int_distribution<int> uniintdist(0, num_particles-1);
  auto index = uniintdist(rand_gen);
